		...
		for (File file : listOfFiles)
		{
			if (file.isFile())
			{
				if (file.getName().equalsIgnoreCase("armor.xml"))
				{
					...
					Game.getCurrent().setArmorList(RunXMLParser.parseArmor(mapRootPath + File.separator + file.getName()));
				}

				else if (file.getName().equalsIgnoreCase("weapons.xml"))
				{
					logger.info("parsing weapons: {}", mapRootPath + File.separator + file.getName());
					Game.getCurrent().setWeaponList(RunXMLParser.parseWeapons(mapRootPath + File.separator + file.getName()));
				}

				else if (file.getName().equalsIgnoreCase("utilities.xml"))
				{
					logger.info("parsing utilities: {}", mapRootPath + File.separator + file.getName());
					Game.getCurrent().setUtilityList(RunXMLParser.parseUtilities(mapRootPath + File.separator + file.getName()));
				}

				else if (file.getName().equalsIgnoreCase("furniture.xml"))
				{
					logger.info("parsing furniture: {}", mapRootPath + File.separator + file.getName());
					Game.getCurrent().setFurnitureList(RunXMLParser.parseFurniture(mapRootPath + File.separator + file.getName()));
				}
		...
		for (File file : listOfFiles)
		{
			if (file.isFile())
			{
				if (file.getName().equalsIgnoreCase("npc.xml"))
				{
					...
					Game.getCurrent().setNpcList(RunXMLParser.parseNPCs(mapRootPath + File.separator + file.getName()));
				}
			}
		}
		logger.info("end: initialize items");
